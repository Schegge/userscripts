...
(async function() {
   ...
   if (await Check.version()) {
      /*document.body.insertAdjacentHTML('beforeend',
         ...
         <b>AO3: Fic's Style, Blacklist, Bookmarks</b> UPDATES (v3.6.1.3)<br><br> 
         <br><br>
   ...
   if (Feature.black && Check.black()) {
      ...
      const Blacklist = {
         ...
         findFandoms: function(work) {
            return this.opts.maxFandoms &&
               work.querySelectorAll('.header .fandoms .tag').length > this.opts.maxFandoms;
         },
         ...
         getWords: function(work) {
            if ((this.opts.minWords || this.opts.maxWords) && work.querySelector('dd.words')) {
               ...
               if (this.opts.minWords &&
                   numWords < this.opts.minWords ||
                   this.opts.maxWords &&
                   numWords > this.opts.maxWords) {
         ...
         setVisibility: function() {
            ...
            for (let work of document.querySelectorAll(this.blurb)) {
               ...
               if (this.findFandoms(work)) reasons.unshift('Fandoms');
            ...
            blackMenu.innerHTML = `<a>Blacklist</a>
               <ul class="menu dropdown-menu">
                  <li>
                     ...
                     <span>fandoms<br>max
                        <input id="${SN}-black-maxFandoms" type="number" min="0" step="1" value="${
                           this.opts.maxFandoms}"></span>
