...
(function($) {
    ...
    function checkNewChapters(element) {
        ...
        if ( followPage ) {
            $("#corpo > div:nth-child(1) > div:nth-child(3)").before(
                "<div style='margin: 0px 10px 10px 15px; padding: 0 10px 10px 10px; color: #009999;'>" +
                alert + "</div>");
        } else if ( segnale ) {
            var menuWidth = parseInt( $("#secondmenu").css("width") );
            $("#secondmenu").prepend(
                "<a id='cuore' style='color: #009999' href='http://www.efpfanfic.net/followed.php'>&#9825;</a> " +
                "<div id='alert' style='display: none; position: absolute; color: #009999; background-color: #fff; padding: 5px; " +
                "font-size: 10px; line-height: 14px; text-align: left; box-shadow: 0 0 3px 0 rgba(91, 146, 178, .5); border-radius: 3px; margin-top: 5px; width: " + 
                menuWidth + "px;'>" + alert + "</div>"
            );
